...
static void pad_added_handler(GstElement *src, GstPad *new_pad, Pipeline *data)
{
    ...
    if (g_str_has_prefix(new_pad_type, "video/x-raw") || g_str_has_prefix(new_pad_type, "video/"))
    {
        /* This is a video pad */
        ...
        /* Attempt the video link */
        ret = gst_pad_link(new_pad, sink_pad);
        if (GST_PAD_LINK_FAILED(ret)) {
            g_print("  Video link failed (type '%s').\n", new_pad_type);
        } else {
            g_print("  Video link succeeded (type '%s').\n", new_pad_type);
        }
    }
    else if (g_str_has_prefix(new_pad_type, "audio/x-raw") || g_str_has_prefix(new_pad_type, "audio/")) {
        /* This is an audio pad */
        ...
        ret = gst_pad_link(new_pad, sink_pad);
        if (GST_PAD_LINK_FAILED(ret)) {
            g_print("  Audio link failed (type '%s').\n", new_pad_type);
        } else {
...
{
    // strcpy(pipe->stream_uri,"file:///home/user/input.mkv");
    strcpy(pipe->stream_uri,"file:///home/user/Videos/input.mp4");
    pipe->loop = true;
    ...
    // Audio elements
    pipe->audioconvert = gst_element_factory_make("audioconvert", "audioconvert0");
    pipe->audioresample = gst_element_factory_make("audioresample", "audioresample0");
    pipe->audiosink = gst_element_factory_make("autoaudiosink", "audiosink0");
    
    if (!pipe->pipeline || !pipe->videosrc || !pipe->convert || !pipe->scale || !pipe->appsink ||
        !pipe->audioconvert || !pipe->audioresample || !pipe->audiosink)
    {
        g_printerr("Not all elements could be created.\n");
        return false;
    }
    g_object_set(pipe->videosrc, "uri", pipe->stream_uri, NULL);
   g_object_set(pipe->scale, 
    "add-borders", TRUE,    // Maintain original ratio with borders
    "method", 1,            // Good quality scaling
    NULL);
    // Add all elements to the pipeline
    gst_bin_add_many(GST_BIN(pipe->pipeline), 
                     pipe->videosrc, pipe->convert, pipe->scale, pipe->appsink,
                     pipe->audioconvert, pipe->audioresample, pipe->audiosink,
                     NULL);
    ...
    // Link audio elements
    link_ok = gst_element_link_many(pipe->audioconvert, pipe->audioresample, pipe->audiosink, NULL);
    g_assert(link_ok);
        ...
        igNewFrame();
        {
                        ...
                        ImGuiWindowFlags_NoTitleBar);
            {
                ...
                igBeginTabBar("main_tab", 0);
                {
                    ...
                    {
                       // GstSample *videosample = gst_app_sink_pull_sample(GST_APP_SINK(ImPipe.appsink));
                        GstSample *videosample =
                        gst_app_sink_try_pull_sample(GST_APP_SINK(ImPipe.appsink),GST_SECOND);
                        ...
                        if (videosample)
                        {
                            ...
                            glBindTexture(GL_TEXTURE_2D, videotex);
                            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,400,400, 0, GL_RGBA,
                             GL_UNSIGNED_BYTE, map.data);

                            gst_buffer_unmap(videobuf, &map);
                            gst_sample_unref(videosample);

                            ImTextureRef tex_ref = {._TexData = NULL, ._TexID = (ImTextureID)videotex};

                            ImDrawList* draw_list = igGetWindowDrawList();
                            igImage(tex_ref, (ImVec2){400,400}, (ImVec2){0,0}, (ImVec2){1,1}); // Correct UV coordinates
                        }
